...
type IssueFormProps = {
  onClose: () => void
  onSubmit: (description: string, photo: File) => Promise<void>
}

const MIN_CHARS = 10
const MAX_CHARS = 500
const POINTS = 15 // Always 15 since photo is required
...
export default function IssueForm({ onClose, onSubmit }: IssueFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!description || description.length < MIN_CHARS) {
      setError(`Intel too short. Need at least ${MIN_CHARS} characters.`)
      return
    }

    if (!photo) {
      setError('Evidence photo is required to report an issue.')
      return
    }
  ...
  const canSubmit = charCount >= MIN_CHARS && !!photo && !loading
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 bg-slate-900 text-white rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(16,185,129,0.15)] p-6 pb-24 z-10 animate-in slide-in-from-bottom duration-300 border-t-4 border-emerald-500">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="relative">
          ...
          {preview ? (
            <div className="relative w-full h-44 rounded-2xl overflow-hidden border-2 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)] group">
              <div className="absolute inset-0 border-4 border-emerald-500/20 z-10 pointer-events-none rounded-2xl" />
              ...
              {!loading && (
                <button
                  ...
                  className="absolute top-2 right-2 bg-slate-900/80 backdrop-blur-md text-white p-2 rounded-xl hover:bg-red-500 transition-all z-20"
                >
            ...
            <button
              ...
              className="w-full py-7 flex flex-col items-center justify-center border-2 border-dashed border-red-500/40 rounded-2xl hover:border-emerald-400 hover:bg-emerald-400/5 transition-all group disabled:opacity-50 relative overflow-hidden bg-red-500/5"
            >
              ...
              <span className="text-xs font-black text-red-400 mt-1 bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20">
                Required
              </span>
            </button>
          )}
        ...
        {/* Helper text when submit is blocked */}
        {!photo && charCount >= MIN_CHARS && (
          <p className="text-center text-[10px] font-black text-red-400/70 uppercase tracking-widest">
            Evidence photo required to deploy
          </p>
        )}
      </form>
    </div>
